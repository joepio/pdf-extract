...
fn get_catalog(doc: &Document) -> Res<&Dictionary> {
    if let Object::Reference(id) = doc.trailer.get(b"Root")? {
    ...
    fn new(doc: &'a Document, font: &'a Dictionary) -> Res<PdfCIDFont<'a>> {
        ...
        let encoding = match &encoding {
            ...
            Object::Stream(stream) => {
                ...
                dlog!(
                    "Stream: {}",
                    std::str::from_utf8(&contents).map_err(|_| "bad utf8")?
                );
    ...
    let space = match name {
        ...
        _ => {
            ...
            if let Ok(cs) = cs.as_array() {
                ...
                match cs_name.as_ref().unwrap().as_ref() {
                    "Separation" => {
...
pub fn output_doc(doc: &Document, output: &mut dyn OutputDev) -> Res<()> {
    if doc.trailer.get(b"Encrypt").is_ok() {
        return Err("Encrypted documents are not currently supported: See https://github.com/J-F-Liu/lopdf/issues/168".into());
    }
